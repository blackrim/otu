...
import opentree.otu.NexsonWriter;
...
public class studyJsons extends ServerPlugin {
	...
	@Description("Return a nexson for the study")
	@PluginTarget( GraphDatabaseService.class )
	public String getNexsonFromStudyID(@Source GraphDatabaseService graphDb,
			@Description ("study ID")
			@Parameter(name = "studyID", optional = false) String studyID){
		DatabaseManager dm = new DatabaseManager(graphDb);
		try {
			return NexsonWriter.writeNexson(studyID,dm);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			return "failed";
		}
